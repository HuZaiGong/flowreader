...
| **HTML 解析** | JSoup 1.16.2 |
| **测试框架** | JUnit 4 + MockK 1.13.9 |
...
│   │   ├── ui/                 # 表现层
│   │   │   ├── screens/        # 页面 (library, reader, bookdetail, settings)
│   │   │   ├── theme/          # 主题样式
│   │   │   └── Navigation.kt   # 导航配置
│   │   ├── util/               # 工具类
│   │   │   ├── BookParser.kt   # 书籍解析
...
### 命名注意
- `FlowReaderApplication` - Application 类 (`FlowReaderApp.kt`)
- `FlowReaderRoot` - 根 Composable 函数
- 存在命名碰撞历史：旧代码中 `FlowReaderApp` 曾用于两者
...
- **命名碰撞：** `FlowReaderApp` 历史上用于 Application 和 Composable 两个不同实体
- **CI 权限：** GitHub Actions 使用较宽的 `contents: write` 权限
- **UseCase 层：** 部分用例已实现，但尚未完善
- **缺少代码检查工具配置：** 无 detekt、lint.xml 配置文件
