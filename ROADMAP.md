...
- [ ] **`:core` 组件库补全**：`FlowScaffold`/`FlowTopBar`/`BookCover`/`SkeletonBox` 等，配 `@Preview`（浅/深/大字号/RTL）
- [ ] **骨架屏替换转圈**：书架冷启动首屏
- [ ] **字符串外置第一批**：书架 + 设置迁入 `strings.xml`，四语言补齐并实现应用内语言切换（当前 `stringResource` 使用率为 0）
...
- [ ] **书架双视图**：网格/列表切换 + 程序化封面 + 继续阅读大卡
- [ ] **独立搜索目的地**：两段结果 + 历史 + 分页（不再截断）
- [ ] **自适应导航**：`NavigationSuiteScaffold`，Compact/Medium/Expanded 三档
- [ ] **自定义主题编辑器**：用户自定义阅读背景色/文字色（本次重构唯一的 Room 迁移点 `MIGRATION_6_7`）
- [ ] **阅读卡片分享**：生成卡片（书名 + 当前页 + 进度），分享到系统剪贴板/其他 App
