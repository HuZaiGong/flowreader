...
> 本版本对全项目搜索功能实施整体整改。经核查，中文全文检索自 FTS 索引建立以来即处于不可用状态，另有索引生命周期、查询转义、历史记录存储等 10 项独立缺陷。本次一并处理，并补充可执行真实 FTS5 查询的测试手段。

### 修复

**事项一：中文全文检索恒返回空结果（根本原因）**

经核查，`book_content_fts` 建表时未指定 `tokenize`，默认采用 `unicode61` 分词器。该分词器按 Unicode 空白与标点切分，不识别 CJK 词边界，一段连续汉字将整体存为单个 token。代码发出的查询形式为短语加前缀，前缀仅能匹配 token 起始位置，故除恰好位于句首或标点之后的词以外，中文查询一律无结果。以「这是一个关于心流阅读的故事」为例，检索「心流」「阅读」「故事」均为 0 条，检索「江南」因其位于标点之后而侥幸命中。

书内搜索与全库章节搜索共用同一索引，两个入口同时失效。v56.4.3 修正的 `CAST(? AS INTEGER)` 仅解决了书内搜索的书籍范围过滤，分词层面的问题于其后显现。

处理措施：新增 `CjkTokenizer`（`:core`），在写入索引前于应用层完成分词。先经 `fold()` 仅保留字母与数字并转小写，再于折叠后的码点流上切分重叠二元组（bigram）。查询经同一路径处理并作为 FTS5 短语匹配，短语的位置连续性约束保证「海上上海」不会命中「上海海上」。

先折叠再分词使匹配可跨越标点与文字种类：「事，江」「流Flow」「海1949」均可命中，英文一并获得词中匹配能力（`eading` 命中 `reading`）。折叠同时构成结构性转义，任何 FTS5 操作符字符均无法存活至查询串，查询转义不再依赖字符黑名单。

已知代价：索引流长度约为折叠后 CJK 字符数的 3 倍。所有承载原文的列均声明为 `UNINDEXED`，该倍数不作用于可读文本。

未采用 FTS5 自带 `trigram` 分词器，原因为该分词器要求查询不少于 3 个字符，而中文最常用的检索词为二字词；且其稳定可用需 SQLite 3.34 以上，本项目 `minSdk 26` 无法覆盖。

**事项二：单个汉字无法检索**

经核查，「爱」「山」等单字为正常检索词，但原有「不少于两个字符」的门槛使输入单字后不发出请求、不返回结果、亦无任何提示。

处理措施：改由 `CjkTokenizer.isSearchableQuery()` 判定。单个非 ASCII 字符照常检索；单个拉丁字母或数字仍予拒绝，因其将退化为匹配大半书库的前缀查询。

**事项三：输入过程中提示「搜索失败」**

经核查，`CancellationException` 继承自 `Exception`，而防抖任务每次按键均取消上一次执行，该异常被通用的 `catch (Exception)` 捕获并按失败上报。

处理措施：三处搜索路径均先行 `catch (CancellationException) { throw e }` 予以放行。

**事项四：首次搜索触发全库索引重建**

经核查，`searchChapters()` 将书库 id 集合与一个进程内存标志比对，该标志无法跨进程存活，故每次启动后的首次搜索均在搜索界面的防抖任务内同步重解析全库；而该任务在下一次按键时即被取消，索引因此始终无法建成。

处理措施：索引状态改由索引库自身的 `indexed_books` 完成标记表记录，稳定状态下仅需两次查询，被中断的运行按已完成部分续跑而非重来。维护工作移至新增的 `@ApplicationScope` 进程级作用域，离开界面不再丢弃半成品索引；由 `requestIndexMaintenance()` 触发，`jobMutex` 保证并发调用汇入同一次运行。

**事项五：阅读器每次打开书籍均重新索引**

经核查，`ReaderViewModel` 自带一份删除后重建的循环逻辑，既使索引知识分散于两处，又从不写入完成标记，维护逻辑随后将同一本书再解析一遍。重开一本 500 章的书籍即重新索引 500 章。

处理措施：统一改由 `SearchRepository.indexBook(bookId, force = false)` 处理。

**事项六：备份恢复后搜索长期返回错误书籍的正文**

经核查，`insertBook` 采用 `onConflict = REPLACE`，恢复过程可将另一本书写入书库已使用过的 id；此时索引中该 id 的文本与完成标记仍属旧书，维护逻辑据完成标记跳过该书，搜索遂以新书标题呈现旧书正文，且该状态可长期存续。

处理措施：导入事务提交后调用 `FullTextSearch.deleteBooks()` 使相应索引失效。该调用有意置于 Room 事务之外：索引位于另一数据库，持有事务期间再获取索引锁将引入锁序风险。

**事项七：`LIKE` 通配符未转义**

经核查，`%` 与 `_` 在绑定参数中同样具备通配符语义，检索「%」将返回全部藏书，检索 `a_c` 会命中 `abc`。

处理措施：`BookDao.searchBooks` 与 `getBooksByTag` 声明 `ESCAPE '\'`，参数经 `SqlLike.escape()` 转义。

**事项八：搜索历史被自身分隔符破坏**

经核查，历史记录以 `"|"` 直接拼接存储，含 `|` 的查询在下次读取时裂为两条无效记录，且重新编码后该损坏永久保留。

处理措施：新增 `SearchHistory`（`:core`）负责转义编解码。同时修正排序规则：重复查询现提升至队首，此前的忽略处理会使最近一次搜索停留在第 10 位。

**事项九：删除书籍未清理索引**

经核查，全库搜索在书籍删除后继续返回已无法打开的命中项。

处理措施：三条删除路径统一调用 `FullTextSearch.deleteBooks()`。

### 新增

**索引进度可见**

新增 `SearchIndexProgress` 与 `SearchRepository.indexProgress`。搜索界面显示「正在建立全文索引 N/M」，结果不完整时另行提示「索引尚未完成，章节结果可能不全」，并在索引完成时自动重跑当前查询；否则建索引期间发出的查询将长期停留在部分结果上，只能由用户重新输入。相关文案已完成 9 种语言翻译。

**命中高亮**

索引现返回命中在摘要中的偏移与长度（`matchStart` / `matchLength`），全库搜索与阅读器内搜索均以 `AnnotatedString` 渲染强调，原有的字面 `<<` `>>` 标记予以移除。命中仅存在于归一化流中时（如跨标点匹配）偏移为 -1，此时按原文渲染，不作偏移错位的高亮。

### 变更

`FullTextSearch.deleteBooks(ids)` 改为自行获取索引锁，调用方不必再判断自身是否已持有该锁。该锁不可重入，判断错误即为死锁。

移除 `AnnotationDao` 中两个无生产调用方的 `LIKE` 笔记搜索方法及其仓库层链路。笔记界面本就持有全部批注并在内存中作大小写无关过滤，SQLite 的 `LIKE` 对非 ASCII 字符无此能力，两套语义并存且不一致。

`flowreader_fts.db` 改用 `PRAGMA user_version` 记录 schema 版本。索引属纯派生数据，版本变更即丢弃重建；连带清空 `indexed_books` 正是使重建得以发生的条件。

### 测试

- 新增 `FullTextSearchEngineTest`：纯 JVM 测试，经 `org.xerial:sqlite-jdbc` 对真实 FTS5 执行仓库中实际发布的 DDL 与 SQL 常量（非复述副本），并按 `rawQuery()` 的方式仅绑定 `String`。覆盖句中中文检索、跨标点与跨文字种类及跨数字匹配、短语语义、`CAST(? AS INTEGER)` 书籍范围过滤、摘要窗口与高亮偏移、重建索引后的 `'integrity-check'`、完成标记语义。
  - 据此更正 `AGENTS.md` 与 v56.4.3 条目所载「无测试可执行这些查询」的结论：Robolectric 自带 SQLite 确无 fts5 模块，但 sqlite-jdbc 有。仅断言 SQL 字符串形状的测试对分词层面的缺陷完全免疫，此即事项一得以存续至今的原因。
- 新增 `SearchRepositoryImplTest`（13 项，索引生命周期）、`SearchViewModelTest`（17 项，防抖、取消、进度重跑、分页、历史）、`SearchHistoryTest`、`HighlightedSnippetTest`、`SearchIndexProgressTest`，并扩充 `CjkTokenizerTest` 与 `BookRepositoryImplTest`。
- `SearchRepositoryImplTest.concurrentRequestsShareASingleRun` 以 `awaitIndexReady()` 而非 `advanceUntilIdle()` 等待：自 coroutines-test 1.7 起调度器仅推进前台任务，fire-and-forget 的维护任务不会运行，断言将以错误的理由通过。
- 测试广度：85.3% → 91.7%（66/72）。
